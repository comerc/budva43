...
5. **Функциональность по требованию**
...
## Создание и обработка ошибок (deprecated)

1. **Типы ошибок**:
   - Ошибки домена (`domain/errors.go`)
...
3. **Сущности как простые структуры данных** - сущности (domain) должны содержать только структуры данных, конструкторы и простые геттеры/сеттеры, без сложной бизнес-логики.
...
### UPD Решение через Function Types

Идея заключается в создании типа функции, который сам реализует общий интерфейс. 

```go
// Создаём общий интерфейс
type Handler interface {
    Handle(driver1)
}

// Создаём тип функции
type HandlerFunc func(driver1)

// Реализуем интерфейс для функции
func (hf HandlerFunc) Handle(d driver1) {
    hf(d)
}

// Теперь это работает:
var f1 Handler = HandlerFunc(func(d driver1) { d.Method() })
var f2 Handler = f1 // НЕТ ошибки!
```

Это паттерн, который активно используется в стандартной библиотеке Go, например в http.HandlerFunc. Позволяет обойти ограничение типизации Go и использовать интерфейсы в аргументах методов других интерфейсов. Но требует публичный тип для интерфейса в аргументах, чтобы выполнить генерацию в mockery. А потому не используется в данном проекте.
