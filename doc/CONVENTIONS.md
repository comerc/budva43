...
### **Схема префиксов:**
- `100x` - общие тестовые данные (переиспользуемые)
- `101x` - TestTransformService 
- `102x` - TestEngineService
- `103x` - TestForwarderService
- `104x` - TestUpdateMessageHandler
- ...

### **Структура номера:**
- **3 цифры** = условный номер модуля
- **1 цифра** = индекс внутри теста (0-9)

### **Ограничения:**
- Максимум **100 модулей** (100-199)
- Максимум **10 значений** на тест
- Максимум **10 тестов** на модуль (при табличном подходе)

### **Правила:**
1. **Комментарий в начале** тестового файла: `// TestTransformService - 101x`
2. **Комментарий в config.yml**: `# TestTransformService - 101x`
3. **Статическая конфигурация** - никаких динамических подмен в тестах
4. **Изоляция данных** - каждый модуль использует свой диапазон
