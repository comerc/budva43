Feature: 12.2.copy_once_f

  Background:
    Given будет пересылка - копия

  Scenario Outline: Сообщение обновляется в целевом чате
    Given исходный чат "<src_chat_name>" (<src_chat_id>)
    # "INCLUDE" побочно, чтобы выполнить forward-rules
    And сообщение с текстом "INCLUDE some text"
    ...
    # "INCLUDE" побочно, чтобы выполнить forward-rules
    And сообщение с текстом "INCLUDE some OTHER text"
    And будет текст "some OTHER text"
