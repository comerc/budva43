...
func (s *Service) addSourceSign(formattedText *client.FormattedText, title string) error {
	sourceSign, err := s.telegramRepo.GetClient().ParseTextEntities(&client.ParseTextEntitiesRequest{
		Text: title,
	...
	if err != nil {
		s.log.Error("ParseTextEntities", "err", err)
	} else {
	...
	s.log.Info("addSourceSign", "formattedText", formattedText)
	return nil
}
